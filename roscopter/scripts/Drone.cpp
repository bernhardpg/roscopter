...
//  Vector3<Expression> dVel = Vector3<Expression>(r*v-q*w, p*w-r*u, q*u-p*v) + Vector3<Expression>(-g*sin(theta), g*cos(theta)*sin(phi), g*cos(theta)*cos(phi)) + (1/m)*Vector3<Expression>(0,0,-f);
//
...
//  Vector3<Expression> dAngleVel = Vector3<Expression>((1/ixx)*t_phi, (1/iyy)*t_theta, (1/izz)*t_gamma);
  ...
  Vector3<Expression> dVel = Vector3<Expression>(-g*theta, g*phi, -f/m);
